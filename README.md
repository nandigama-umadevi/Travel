...
routes:
  list: FromKondor, FromCalypso, FromHubAny, FromPMT

mq:
  host: "{{infra.mq.server.name}}"
  port: "{{infra.mq.provider.server.port}}"
  name: "{{infra.mq.queue.manager}}"
  channel: "{{infra.mq.queue.channel}}"
  user: "{{infra.mq.queue.user}}"
  password: "{{infra.mq.queue.password}}"
  cypher: "{{infra.mq.queue.cypher}}"

queue:
  oktopus:
    to:
      calypso: "jms:{{Q_XGEN_TO_CALYPSO_TRADE}}"
      kondor: "jms:{{Q_CALYPSO_TO_KPLUS}}"
      pmt: "jms:{{Q_CALYPSO_TO_PMT}}"
  calypso:
    to:
      oktopus: "jms:{{Q_CALYPSO_TO_XGEN_TRADE}}"
      oktopus_bck: "jms:{{Q_CALYPSO_TO_XGEN_TRADE_BCK}}"
  kondor:
    to:
      oktopus: "jms:{{Q_KPLUS_TO_CALYPSO}}"
      oktopus_bck: "jms:{{Q_KPLUS_TO_CALYPSO_BCK}}"
  hubany:
    to:
      oktopus: "jms:{{Q_KAPTURE_TO_XGEN_FUT}}"
      oktopus_bck: "jms:{{Q_KAPTURE_TO_XGEN_FUT_BCK}}"
  pmt:
    to:
      oktopus: "jms:{{Q_PMT_TO_CALYPSO}}"
      oktopus_bck: "jms:{{Q_PMT_TO_CALYPSO_BCK}}"

WS_PMT_TRADE_DATA: "{{WS_PMT_TRADE_DATA}}"
WS_PMT_STRUCTURED_PRODUCT: "{{WS_PMT_STRUCTURED_PRODUCT}}"
WS_PMT_FEES_CLIENT: "{{WS_PMT_FEES_CLIENT}}"
WS_PMT_FEES_PRODUCT: "{{WS_PMT_FEES_PRODUCT}}"
WS_PMT_PRODUCT: "{{WS_PMT_PRODUCT}}"
WS_PMT_MTM: "{{WS_PMT_MTM}}"
WS_PMT_TRANSACTION: "{{WS_PMT_TRANSACTION}}"
WS_PMT_LOGIN: "{{WS_PMT_LOGIN}}"
WS_PMT_PASSWORD: "{{WS_PMT_PASSWORD}}"

maks:
  api:
    url: "{{maks.api.host}}/calypso-service/api"

sgconnect:
  auth:
    access_token_uri: "{{SGCONNECT_HOST}}/sgconnect/oauth2/access_token"
    client_id: "{{auth.server.endpoints.client-id}}"
    client_secret: "{{auth.server.endpoints.client-secret}}"

mconnect:
  maks:
    auth:
      scopes: "{{maks.api.auth.scopes}}"
